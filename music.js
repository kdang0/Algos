...
function musicRuntime(busDuration, songDurations) {
    const objectiveTime = busDuration - 30
    let longestSong = songDurations[0]
    let expected = [-1,-1]
    let longestSongIndx = null
    ...
    const goal = objectiveTime - longestSong
    for (let i = 0; i < songDurations.length; i++){
        if (songDurations[i] === goal){
            expected = [i, longestSongIndx]
        }
    }
    return expected
}
